...
def calculate_take_profit_levels(entry_price, stop_loss_price, risk_reward_ratios=[1, 2, 3]):
    ...
    for ratio in risk_reward_ratios:
        if ratio <= 0:
            raise ValueError("Risk:reward ratios must be positive")
        if is_long:
            ...
            tp_price = entry_price - (risk * ratio)
            if tp_price <= 0:
                raise ValueError("Short take-profit calculation produced a non-positive price")
...
def format_position_output(result, take_profit_levels, is_long):
    ...
    for tp in take_profit_levels:
        if is_long:
            ...
            profit_amount = (result['entry_price'] - tp['price']) * result['position_size_units']
