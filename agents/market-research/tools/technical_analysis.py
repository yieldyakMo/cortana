...
def calculate_ema_series(prices, period):
    """Return EMA values aligned to prices, with None before the seed value."""
    if len(prices) < period:
...
def calculate_macd(prices, fast=12, slow=26, signal=9):
    ...
    fast_series = calculate_ema_series(prices, fast)
    slow_series = calculate_ema_series(prices, slow)
    macd_history = [
...
def main():
    ...
    try:
        ...
        if args.json:
            ...
            analyze(klines, args.rsi_period, args.bb_period)
