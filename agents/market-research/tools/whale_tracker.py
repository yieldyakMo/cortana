...
def get_recent_trades(symbol, limit=1000):
    ...
    url = endpoint("/api/v3/trades", symbol=validate_symbol(symbol), limit=limit)
    return fetch_json(url)

def get_orderbook(symbol, limit=1000):
    """Get orderbook depth"""
    limit = max(5, min(int(limit), 5000))
    url = endpoint("/api/v3/depth", symbol=validate_symbol(symbol), limit=limit)
    return fetch_json(url)

def get_ticker(symbol):
    """Get 24h ticker for reference"""
    url = endpoint("/api/v3/ticker/24hr", symbol=validate_symbol(symbol))
    return fetch_json(url)
...
def analyze_large_trades(trades, threshold_percentile=90):
    ...
    if not 0 < threshold_percentile <= 100:
        raise ValueError("Threshold percentile must be greater than 0 and at most 100")
...
def format_large_trades_output(large_trades, threshold):
    ...
    if buy_volume > sell_volume * 1.2:
        print(f"  Large recent trades skewed toward buyer-initiated prints")
    elif sell_volume > buy_volume * 1.2:
        print(f"  Large recent trades skewed toward seller-initiated prints")
