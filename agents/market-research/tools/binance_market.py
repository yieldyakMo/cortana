...
def get_price(symbol):
    ...
    url = endpoint(BASE_URL, "/api/v3/ticker/price", symbol=validate_symbol(symbol))
    data = fetch_json(url)
    return data

def get_ticker_24h(symbol):
    """Get 24h ticker statistics"""
    url = endpoint(BASE_URL, "/api/v3/ticker/24hr", symbol=validate_symbol(symbol))
    data = fetch_json(url)
    return data
...
def get_orderbook(symbol, limit=20):
    ...
    url = endpoint(BASE_URL, "/api/v3/depth", symbol=validate_symbol(symbol), limit=limit)
    data = fetch_json(url)
    return data
...
def get_recent_trades(symbol, limit=100):
    ...
    url = endpoint(BASE_URL, "/api/v3/trades", symbol=validate_symbol(symbol), limit=limit)
    data = fetch_json(url)
    return data
...
def get_funding_rate(symbol):
    ...
    try:
        url = endpoint(FAPI_URL, "/fapi/v1/premiumIndex", symbol=validate_symbol(symbol))
        data = fetch_json(url)
        return data
    except:
